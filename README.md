...
- **orelia-core**（任意・softdepend）: 未導入でも起動しますが、導入されている場合はOreliaCoreのレベル/職業/所持金を、サイドバー・タブリスト名色/接頭辞/接尾辞/右側値・名札下・チャットのプレースホルダーに自動反映します（`core-integration.enabled: false`で一括無効化。個別の有効/無効は`scoreboard.core-lines`・`tablist.name-color`・`tablist.value`・`belowname.core-format`・`chat.core-placeholder`各セクションの`enabled`で設定）。
- **orelia-extra**（任意・softdepend）: 未導入でも起動しますが、導入されている場合は`{guild}` `{guild_tag}` `{party}`プレースホルダーが解決されます（未所属時は空文字）。
- **orelia-world**（任意・softdepend）: 未導入でも起動しますが、導入されている場合は`{title}`プレースホルダー（`/ol title equip`で装備中のクエスト称号）が解決されます(未装備時は空文字)。
...
- `rpg.serverutil.api.ChatApi` / `ChatPlaceholderProvider` — チャット送信者の名前の横に表示するプレースホルダー（レベル・称号・ランク等）を差し込む。表示位置・整形は`config.yml`の`chat.format`で制御します。送信者名にマウスホバーすると、送信時刻・プレイヤー名・レベル等のツールチップも表示されます（`chat.tooltip.enabled` / `chat.tooltip.lines`、既定で有効）。`chat.color-codes.enabled`（既定true）の場合、`orelia.serverutil.chat.color`権限（既定op限定）を持つプレイヤーはチャットで`&`カラーコード（レガシー・`&#RRGGBB`hex・OreliaCoreのカスタム`&%<char>`コード全て）をそのまま使えます。権限が無いプレイヤーの入力はバニラ同様、リテラルなテキストとして表示されます。チャットメッセージ中の`@name`は、オンライン中のプレイヤー名と一致すれば`mention.format`（既定`&%6[&%e@{name}&%6]&r`）で色付けされ、対象プレイヤーに`mention.sound`のサウンドが鳴ります（一致しない`@word`はそのままリテラル表示）。`@all`（`mention.all.keyword`で変更可）は全員宛てのメンションになり、`orelia.serverutil.chat.mention.all`権限（既定op限定）が必要です。パブリックチャットのみが対象で、orelia-extraのパーティー/ギルド/管理者チャットは別経路のため対象外です。

実装例は`rpg.serverutil.paper.integration.CoreIntegrationModule`を参照してください。OreliaCoreが導入されている場合、サイドバー（レベル/所持金、`scoreboard.core-lines.lines`でプレースホルダー対応の書式を設定可能）に加え、上記5つのAPI全てにOreliaCoreの職業色・レベル・所持金を反映するプロバイダーを自動登録します。書式・有効/無効は各機能自身のセクション（`scoreboard.core-lines`・`tablist.name-color`・`tablist.value`・`belowname.core-format`・`chat.core-placeholder`）で設定し、`core-integration.enabled`はそれら全体のマスタースイッチです。
...
- `join.server-switch-wait-ticks`（既定20tick、通知が届き次第それより早く確定します）を上限に、切替通知の到着を待ってからメッセージを確定します
